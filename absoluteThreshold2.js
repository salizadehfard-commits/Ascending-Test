...
import { core, data, sound, util, visual, hardware } from 'https://lib.pavlovia.org/psychojs-2026.1.3.js';
const { PsychoJS } = core;
...
function welcomRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of welcomComponents)
...
function ascendingLoopLoopBegin(ascendingLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisAscendingLoop of ascendingLoop) {
...
function testiRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of testiComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function maintestRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of maintestComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
